<template>
  <a-modal v-model:open="open" @ok="handleOk" :ok-text="$t('button.edit')" :title="$t('button.edit')">
    ...
    <div style="height:700px;overflow-y:auto">
      <a-form ref="createTaskForm" :rules="taskCreateRules" :model="task" :label-col="labelCol" :wrapper-col="wrapperCol"
        ...
        <div v-if="loading">
          ...
          <a-form-item :label="$t('task.type')" name="type">
            <a-select v-model:value="task.type">
              <a-select-option v-for="item in createParam?.taskTypeList"
          ...
          <a-form-item :label="$t('task.assignee')">
            ...
            <UserSelect v-model:userId="task.assignee" :user-opt="userOpt" :onlyProjectUser="true"
              :projectId="task.projectId"></UserSelect>
          </a-form-item>
...
function loadCreateParams() {
  ...
  task.value = cloneTask(edittask.value) ;
